# 設計書

## プロジェクト概要

**複数占い手法統合システム（Multi-Method Fortune Telling System）**

データベース駆動型のアーキテクチャで、5つ以上の占い手法を統合し、多角的な運勢分析を提供する占いアプリケーション。

---

## 1. 技術スタック

### バックエンド
- **言語**: Python 3.x
- **Webフレームワーク**: Flask
- **テンプレートエンジン**: Jinja2
- **データベース**: SQLite
- **ORM**: SQLAlchemy（推奨）

### フロントエンド
- **UI**: シンプルなHTML/CSS + JavaScript
- **視覚化**: HTML/CSS プログレスバー、カラーコーディング
- **将来的な拡張**: Chart.js（レーダーチャート）

### 占い計算ライブラリ
- **西洋占星術**: pyswisseph（Swiss Ephemeris）
- **その他**: 標準ライブラリ + カスタム実装

### 将来的な拡張
- **LLM統合**: Claude API または OpenAI API（総合判断の自然文生成）

---

## 2. MVPスコープ（Phase分け）

### Phase 1（最小MVP） - 初期実装
- ✅ ユーザー情報入力
  - **生年月日**（必須）
  - **出生時刻**（任意、不明な場合は正午をデフォルト使用）
  - **出生地**（任意、不明な場合は東京をデフォルト使用）
- ✅ 5つの占い手法による分析
  - 西洋占星術（出生時刻・場所があればハウス・アセンダント計算）
  - タロット占い（完全ランダム、3枚引き）
  - 数秘術
  - 四柱推命（出生時刻があれば時柱計算）
  - 易経
- ✅ 個別結果と総合判断の表示
  - 各手法の個別結果
  - 計算根拠の表示（最小限）
  - 総合スコアと評価
  - 共通点・相違点の分析
- ✅ 運勢カテゴリー別分析（恋愛・仕事・健康・金運）
- ✅ シンプルなHTML/CSS視覚化（プログレスバー、カラーコーディング）
- ✅ 履歴保存機能（最小限実装）
  - 結果の保存
  - 過去の結果一覧表示
  - 過去の結果詳細表示
- ✅ 入力バリデーション

### Phase 2（機能拡張）
- 視覚化の強化（Chart.js によるレーダーチャート）
- 計算根拠の詳細説明（歴史的背景、理論の解説）
- LLM統合（自然な解釈文生成）

### Phase 3（管理機能）
- 管理画面（占い手法のデータベース管理）
- 新規手法の追加・更新・無効化機能

---

## 3. 占い手法の実装レベル

### 方針
**オプションB（中程度版）+ 伝統的な占術理論に忠実**

### 各手法の実装詳細

#### 3.1 西洋占星術
- 主要天体の位置計算（太陽、月、水星、金星、火星、木星、土星）
- 12ハウスシステム（Placidusハウス）
- 主要アスペクト（合、セクスタイル、スクエア、トライン、オポジション）
- **使用ライブラリ**: pyswisseph
- **計算基準**: 生年月日から出生図（ネイタルチャート）の簡易版を作成

#### 3.2 タロット占い
- **カード構成**: 大アルカナ22枚 + 小アルカナ56枚（計78枚）
- **スプレッド**: 3枚引き（過去・現在・未来）
- **解釈**: 正位置・逆位置の両対応
- **カテゴリー別意味**: 恋愛・仕事・健康・金運ごとに異なる解釈
- **データ管理**: データベースに全カードの意味を格納
- **ランダム性**: 完全ランダム（毎回異なるカードが引かれる）
  - 同じ人が同じ日に複数回占っても、異なる結果が出る可能性がある
  - Pythonの`random.choice()`を使用してカードを選択

#### 3.3 数秘術
- **ライフパスナンバー**（誕生数）: 生年月日の数字を還元
- **パーソナルイヤーナンバー**: 現在の年運
- **マスターナンバー**: 11, 22, 33の特別扱い
- **計算ロジック**: 各桁の和を1桁になるまで還元（マスターナンバー除く）

#### 3.4 四柱推命
- **四柱計算**: 年柱・月柱・日柱・時柱の天干地支
- **十干十二支**: 正確な暦計算
- **通変星**: 比肩、劫財、食神、傷官、偏財、正財、偏官、正官、偏印、印綬
- **五行バランス**: 木火土金水の分析
- **相性関係**: 刑冲破害合の分析
- **データ管理**: 十干・十二支・通変星のマスターデータをデータベースに格納

#### 3.5 易経
- **卦の体系**: 64卦
- **卦の生成**: 筮竹法のシミュレート（確率分布は正確に）
- **解釈**: 本卦と之卦（変爻がある場合）
- **カテゴリー別意味**: 各卦の恋愛・仕事・健康・金運の解釈
- **データ管理**: 64卦と8つの基本卦（八卦）をデータベースに格納

---

## 4. 総合判断のアルゴリズム

### 方針
**カテゴリー別スコアリング + 重み付け平均**

### アルゴリズム詳細

#### 4.1 個別手法のスコアリング
各占い手法が運勢カテゴリーごとにスコアを算出：
- **範囲**: 0〜100点
- **カテゴリー**: 恋愛運、仕事運、健康運、金運

#### 4.2 総合スコア計算
```python
総合スコア = Σ(各手法のスコア) / 手法数
```

例（恋愛運）:
- 西洋占星術: 85点
- タロット: 60点
- 数秘術: 75点
- 四柱推命: 70点
- 易経: 80点
- **平均**: 74点

#### 4.3 評価レベル変換
スコアから評価テキストへの変換：
- 90-100点: "非常に良好"
- 75-89点: "良好"
- 60-74点: "普通"
- 40-59点: "やや注意"
- 0-39点: "注意が必要"

#### 4.4 共通点・相違点の分析（Phase 1実装）

各占い手法の結果を比較し、共通点と相違点を分析します。

##### 分析アルゴリズム

1. **標準偏差の計算**
   ```python
   標準偏差(σ) = √[Σ(各手法のスコア - 平均スコア)² / 手法数]
   ```
   - σ < 10: 「全ての手法が一致した見解を示しています」
   - 10 ≤ σ < 20: 「概ね一致した見解です」
   - σ ≥ 20: 「手法間で見解が分かれています」

2. **最高・最低スコアの特定**
   ```python
   最高スコアの手法: 「○○占術(XX点)が最も良好な結果を示しています」
   最低スコアの手法: 「△△占術(YY点)は慎重な判断を示しています」
   ```

3. **カテゴリー間の比較**
   ```python
   各カテゴリーの平均スコアを比較し、最も高い/低いカテゴリーを特定
   例: 「恋愛運(85点)と金運(82点)が高い一方、仕事運(65点)は普通です」
   ```

4. **一致度の表示**
   ```python
   # 各カテゴリーで70点以上をつけた手法の数をカウント
   一致度 = (高評価をつけた手法数 / 全手法数) × 100

   - 80%以上: 「ほとんどの手法が高評価」
   - 50-79%: 「多くの手法が好意的」
   - 20-49%: 「意見が分かれています」
   - 20%未満: 「慎重な見方が多数」
   ```

##### 出力例
```
【共通点】
- 全5つの手法が「恋愛運」について好意的な見解を示しています
- 特に西洋占星術(85点)とタロット(80点)が高評価です

【相違点】
- 仕事運については手法間で意見が分かれています（標準偏差: 18.5）
- 四柱推命(45点)は慎重な見方ですが、数秘術(75点)は良好と判断しています
```

#### 4.5 計算根拠の表示（Phase 1実装）

各占い手法の結果に対して、最小限の計算根拠を表示します。

**西洋占星術:**
```
太陽星座: 牡羊座（火のエレメント）
月星座: 天秤座（風のエレメント）
金星: 双子座 → 恋愛運に影響
火星: 獅子座 → 仕事運に影響
```

**タロット:**
```
過去: 愚者（正位置）- 新しい始まり
現在: 恋人（正位置）- 選択と調和
未来: 太陽（正位置）- 成功と喜び
→ 全体的に前向きな流れ
```

**数秘術:**
```
ライフパスナンバー: 7（内省と精神性）
パーソナルイヤー: 3（表現と創造性）
→ 内面の成長と外面の表現のバランス
```

**四柱推命:**
```
年柱: 甲子（木-水）
月柱: 丙寅（火-木）
日柱: 戊午（土-火）
時柱: 庚申（金-金）
通変星: 食神が強い → 創造性と仕事運に吉
五行バランス: 木火土金水が揃っている
```

**易経:**
```
得た卦: 乾為天（第1卦）
上卦: 乾（天）
下卦: 乾（天）
→ 強い創造力と指導力を示す大吉の卦
```

#### 4.6 将来的な拡張（Phase 2以降）
- **LLM統合**: Claude APIやOpenAIを使用し、各手法の結果を踏まえた自然な解釈文を生成
- **矛盾の処理**: 手法間で大きく異なる結果が出た場合の調整ロジック
- **重み付け**: 手法ごとに得意分野に応じた重み付け

---

## 5. ユーザー認証

### 方針
**認証なし**

### 理由
- ローカル環境での個人使用を想定
- シンプルさを優先
- セットアップの簡易化

### 将来的な拡張（Phase 2以降）
- 簡易ユーザー管理（名前のみ）を追加し、履歴を個人別に管理する選択肢

---

## 6. データベーススキーマ設計

### 6.1 命名規則

#### テーブル名
```
{手法名}_{データ種別}

例:
- tarot_cards
- shichu_stems
- iching_hexagrams
- fortune_methods（共通）
- analysis_results（共通）
```

#### カラム名
```
- 日本語名: name_ja
- 英語名: name_en
- 中国語名: name_zh
- 運勢カテゴリー別: {category}_meaning または {category}_trait
  例: love_meaning, work_trait
- スコア: {category}_score
- フラグ: is_{状態}
  例: is_active, is_reversed
- 日時: {action}_at
  例: created_at, updated_at
```

### 6.2 テーブル定義

#### 共通テーブル

##### fortune_methods（占い手法マスター）
```sql
CREATE TABLE fortune_methods (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name_ja TEXT NOT NULL,
    name_en TEXT NOT NULL,
    description TEXT,
    is_active BOOLEAN DEFAULT 1,
    category TEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
```

##### analysis_results（占い結果保存 - Phase 1で実装）
```sql
CREATE TABLE analysis_results (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    birth_date DATE NOT NULL,
    birth_time TIME,
    birth_place TEXT,
    analysis_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    results_json TEXT,                   -- 全占い結果をJSON形式で保存
    overall_score_love INTEGER,
    overall_score_work INTEGER,
    overall_score_health INTEGER,
    overall_score_money INTEGER
);
```

**Phase 1での使用:**
- 占い結果をJSON形式で保存
- 過去の結果一覧表示機能
- 特定の結果の詳細表示機能

#### 西洋占星術テーブル

##### zodiac_signs（12星座）
```sql
CREATE TABLE zodiac_signs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name_ja TEXT NOT NULL,
    name_en TEXT NOT NULL,
    symbol TEXT,
    element TEXT,                    -- "火", "地", "風", "水"
    quality TEXT,                    -- "活動宮", "不動宮", "柔軟宮"
    ruling_planet TEXT,              -- 支配星
    date_range_start TEXT,           -- "03-21"
    date_range_end TEXT,             -- "04-19"
    love_trait TEXT,
    work_trait TEXT,
    health_trait TEXT,
    money_trait TEXT
);
```

##### astrology_planets（天体）
```sql
CREATE TABLE astrology_planets (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name_ja TEXT NOT NULL,
    name_en TEXT NOT NULL,
    symbol TEXT,
    ruling_sign TEXT,                -- 支配する星座
    meaning TEXT,                    -- 天体の意味
    love_influence TEXT,
    work_influence TEXT,
    health_influence TEXT,
    money_influence TEXT
);
```

##### astrology_houses（ハウス）
```sql
CREATE TABLE astrology_houses (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    house_number INTEGER NOT NULL,   -- 1-12
    name_ja TEXT NOT NULL,
    meaning TEXT,
    life_area TEXT                   -- 担当する人生領域
);
```

##### astrology_aspects（アスペクト）
```sql
CREATE TABLE astrology_aspects (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name_ja TEXT NOT NULL,
    name_en TEXT NOT NULL,
    angle REAL NOT NULL,             -- 0, 60, 90, 120, 180
    orb REAL,                        -- 許容誤差
    aspect_type TEXT,                -- "major" or "minor"
    is_harmonious BOOLEAN,           -- 調和的か否か
    meaning TEXT
);
```

#### タロットテーブル

##### tarot_cards（タロットカード）
```sql
CREATE TABLE tarot_cards (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    card_number INTEGER NOT NULL,    -- 0-77
    name_ja TEXT NOT NULL,
    name_en TEXT NOT NULL,
    arcana_type TEXT NOT NULL,       -- "major" or "minor"
    suit TEXT,                       -- "wands", "cups", "swords", "pentacles"（小アルカナのみ）
    rank TEXT,                       -- "ace", "2"-"10", "page", "knight", "queen", "king"
    upright_general TEXT,
    upright_love TEXT,
    upright_work TEXT,
    upright_health TEXT,
    upright_money TEXT,
    reversed_general TEXT,
    reversed_love TEXT,
    reversed_work TEXT,
    reversed_health TEXT,
    reversed_money TEXT,
    keywords TEXT                    -- キーワード（カンマ区切り）
);
```

#### 数秘術テーブル

##### numerology_meanings（数秘術の数字解釈）
```sql
CREATE TABLE numerology_meanings (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    number INTEGER NOT NULL UNIQUE,  -- 1-9, 11, 22, 33
    is_master_number BOOLEAN DEFAULT 0,
    general_meaning TEXT,
    personality_trait TEXT,
    love_meaning TEXT,
    work_meaning TEXT,
    health_meaning TEXT,
    money_meaning TEXT,
    strengths TEXT,
    weaknesses TEXT
);
```

#### 四柱推命テーブル

##### shichu_stems（十干）
```sql
CREATE TABLE shichu_stems (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    stem_name_ja TEXT NOT NULL,      -- "甲", "乙", "丙", "丁", "戊", "己", "庚", "辛", "壬", "癸"
    stem_name_zh TEXT,
    stem_name_en TEXT,               -- "Kou", "Otsu", etc.
    element TEXT NOT NULL,           -- "木", "火", "土", "金", "水"
    polarity TEXT NOT NULL,          -- "陽", "陰"
    order_num INTEGER NOT NULL,      -- 1-10
    general_meaning TEXT,
    personality_trait TEXT
);
```

##### shichu_branches（十二支）
```sql
CREATE TABLE shichu_branches (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    branch_name_ja TEXT NOT NULL,    -- "子", "丑", "寅"...
    branch_name_zh TEXT,
    branch_name_en TEXT,
    zodiac_animal TEXT,              -- "鼠", "牛", "虎"...
    element TEXT NOT NULL,           -- "水", "土", "木"...
    order_num INTEGER NOT NULL,      -- 1-12
    month_association INTEGER,       -- 対応する月（1-12）
    general_meaning TEXT,
    personality_trait TEXT
);
```

##### shichu_hidden_stems（蔵干）
```sql
CREATE TABLE shichu_hidden_stems (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    branch_id INTEGER NOT NULL,
    hidden_stem_id INTEGER NOT NULL,
    stem_type TEXT,                  -- "本気", "中気", "余気"
    strength INTEGER,                -- 1-3（1が弱、3が強）
    FOREIGN KEY (branch_id) REFERENCES shichu_branches(id),
    FOREIGN KEY (hidden_stem_id) REFERENCES shichu_stems(id)
);
```

##### shichu_tonhensei（通変星）
```sql
CREATE TABLE shichu_tonhensei (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name_ja TEXT NOT NULL,           -- "比肩", "劫財", "食神", "傷官", "偏財", "正財", "偏官", "正官", "偏印", "印綬"
    name_zh TEXT,
    category TEXT,                   -- "比肩", "洩星", "財星", "官星", "印星"
    general_meaning TEXT,
    personality_trait TEXT,
    love_meaning TEXT,
    work_meaning TEXT,
    health_meaning TEXT,
    money_meaning TEXT,
    strengths TEXT,
    weaknesses TEXT
);
```

##### shichu_combinations（干支の相性・刑冲破害）
```sql
CREATE TABLE shichu_combinations (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    combination_type TEXT NOT NULL,  -- "合", "冲", "刑", "破", "害", "三合", "方合"
    element1_type TEXT NOT NULL,     -- "stem" or "branch"
    element1_id INTEGER NOT NULL,
    element2_type TEXT NOT NULL,
    element2_id INTEGER NOT NULL,
    element3_id INTEGER,             -- 三合などの場合
    result_element TEXT,             -- 合化した場合の五行
    effect_description TEXT,
    is_positive BOOLEAN,
    impact_love INTEGER,             -- -100 to 100
    impact_work INTEGER,
    impact_health INTEGER,
    impact_money INTEGER
);
```

#### 易経テーブル

##### iching_trigrams（八卦）
```sql
CREATE TABLE iching_trigrams (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    trigram_number INTEGER NOT NULL, -- 1-8
    name_ja TEXT NOT NULL,           -- "乾", "兌", "離", "震", "巽", "坎", "艮", "坤"
    name_zh TEXT,
    name_en TEXT,
    symbol TEXT,                     -- "☰", "☱", etc.
    binary_code TEXT,                -- "111", "011", etc.
    element TEXT,                    -- 対応する五行
    nature TEXT,                     -- "天", "沢", "火", "雷", "風", "水", "山", "地"
    attribute TEXT,                  -- 属性
    meaning TEXT
);
```

##### iching_hexagrams（64卦）
```sql
CREATE TABLE iching_hexagrams (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    hexagram_number INTEGER NOT NULL UNIQUE, -- 1-64
    name_ja TEXT NOT NULL,
    name_zh TEXT,
    name_en TEXT,
    symbol TEXT,                     -- "䷀" etc.
    trigram_upper_id INTEGER,        -- 上卦（八卦テーブルへの参照）
    trigram_lower_id INTEGER,        -- 下卦
    binary_code TEXT,                -- "111111" etc.
    general_meaning TEXT,
    judgment_text TEXT,              -- 卦辞
    image_text TEXT,                 -- 象伝
    love_meaning TEXT,
    work_meaning TEXT,
    health_meaning TEXT,
    money_meaning TEXT,
    advice TEXT,
    FOREIGN KEY (trigram_upper_id) REFERENCES iching_trigrams(id),
    FOREIGN KEY (trigram_lower_id) REFERENCES iching_trigrams(id)
);
```

##### iching_lines（爻辞）
```sql
CREATE TABLE iching_lines (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    hexagram_id INTEGER NOT NULL,
    line_position INTEGER NOT NULL,  -- 1-6（初爻から上爻）
    line_type TEXT,                  -- "陰" or "陽"
    line_text TEXT,                  -- 爻辞
    meaning TEXT,
    FOREIGN KEY (hexagram_id) REFERENCES iching_hexagrams(id)
);
```

---

## 7. 視覚化方法

### Phase 1の実装
**オプションB: シンプルなHTML/CSS**

#### 実装内容
- **プログレスバー**: 各運勢カテゴリーのスコアを視覚化
- **カラーコーディング**: スコアに応じた色分け
  - 90-100点: 緑（#4CAF50）
  - 75-89点: 黄緑（#8BC34A）
  - 60-74点: 黄色（#FFC107）
  - 40-59点: オレンジ（#FF9800）
  - 0-39点: 赤（#F44336）
- **星評価**: ★★★★☆ のような視覚的表現
- **レスポンシブデザイン**: モバイルでも見やすい

#### HTML/CSS例
```html
<div class="fortune-category">
    <h3>恋愛運</h3>
    <div class="progress-bar">
        <div class="progress-fill" style="width: 74%; background-color: #FFC107;"></div>
    </div>
    <p class="score">74点 / 普通</p>
    <p class="stars">★★★★☆</p>
</div>
```

### Phase 2以降の拡張
- **Chart.js**: レーダーチャート（4つの運勢カテゴリーを一度に表示）
- **グラフ比較**: 各占い手法のスコアを並べて比較
- **時系列グラフ**: 過去の占い結果との比較

---

## 8. 入力バリデーションとデフォルト値処理

### 8.1 入力バリデーションルール

Phase 1で実装する入力バリデーション仕様。

#### 生年月日（必須）

**検証項目:**
- 形式: YYYY-MM-DD または YYYY/MM/DD
- 日付の妥当性チェック
  - 2月30日、4月31日などの存在しない日付は不可
  - うるう年の正確な判定
- 年の範囲: 1900-01-01 ～ 今日の日付
- 未来の日付は不可

**エラーメッセージ:**
```
- "生年月日を入力してください"（未入力時）
- "有効な日付を入力してください"（形式エラー）
- "存在しない日付です"（日付妥当性エラー）
- "1900年以降の日付を入力してください"（範囲外エラー）
- "未来の日付は入力できません"（未来日付エラー）
```

#### 出生時刻（任意）

**検証項目:**
- 形式: HH:MM（24時間形式）
- 時刻の範囲: 00:00 ～ 23:59
- 空欄OK（デフォルト値を使用）

**エラーメッセージ:**
```
- "時刻は00:00～23:59の形式で入力してください"（形式エラー）
```

**補足:**
- 「わからない」チェックボックスを用意
- チェック時は自動的に12:00（正午）をデフォルト値として使用
- 未入力の場合もデフォルト値を使用

#### 出生地（任意）

**検証項目:**
- 形式: 都市名（日本語または英語）または緯度経度
- 例: 「東京」「Tokyo」「札幌」「Osaka」
- 緯度経度の場合: 「35.6895,139.6917」（カンマ区切り）
- 空欄OK（デフォルト値を使用）

**エラーメッセージ:**
```
- "都市名または緯度経度を入力してください"（形式エラー）
```

**補足:**
- 「わからない」チェックボックスを用意
- チェック時または未入力の場合は東京をデフォルト値として使用

### 8.2 デフォルト値の設定

出生時刻・出生地が不明な場合の処理方針。

#### 出生時刻のデフォルト値

**デフォルト値: 12:00（正午）**

**理由:**
- 西洋占星術では出生時刻不明時の標準的な対応
- 1日の中間点であり、統計的に中立
- ハウスの計算で極端な偏りを避けられる

**適用ケース:**
- ユーザーが「わからない」を選択
- 出生時刻フィールドが未入力
- 無効な時刻が入力された場合

**影響を受ける手法:**
- 西洋占星術: ハウスシステム、アセンダント
- 四柱推命: 時柱

**表示:**
```
出生時刻: 12:00 (デフォルト値)
※ 出生時刻が不明なため、正午を使用しています
```

#### 出生地のデフォルト値

**デフォルト値: 東京（経度 139.6917°E、緯度 35.6895°N）**

**理由:**
- 日本のシステムであり、日本の首都を基準とする
- 日本国内の出生が最も多いと想定
- pyswissephで正確な計算が可能

**適用ケース:**
- ユーザーが「わからない」を選択
- 出生地フィールドが未入力
- 無効な地名が入力された場合

**影響を受ける手法:**
- 西洋占星術: 天体の地平位置、ハウスシステム

**表示:**
```
出生地: 東京 (デフォルト値)
※ 出生地が不明なため、東京を使用しています
```

### 8.3 デフォルト値使用時の注意表示

デフォルト値を使用した場合、結果画面に以下の注意書きを表示します。

```
⚠️ 注意事項
この占い結果は、以下のデフォルト値を使用して計算されています：
- 出生時刻: 12:00（正午）
- 出生地: 東京

より正確な結果を得るには、実際の出生時刻と出生地を入力してください。
特に西洋占星術と四柱推命の結果は、出生時刻・場所により大きく変わる可能性があります。
```

### 8.4 入力フォームのUI設計

```html
<form>
  <div class="form-group">
    <label for="birth_date">生年月日 *</label>
    <input type="date" id="birth_date" name="birth_date" required>
    <span class="error-message"></span>
  </div>

  <div class="form-group">
    <label for="birth_time">出生時刻</label>
    <input type="time" id="birth_time" name="birth_time">
    <label>
      <input type="checkbox" id="birth_time_unknown" name="birth_time_unknown">
      わからない（正午を使用）
    </label>
    <span class="error-message"></span>
  </div>

  <div class="form-group">
    <label for="birth_place">出生地</label>
    <input type="text" id="birth_place" name="birth_place"
           placeholder="例: 東京、Tokyo、大阪">
    <label>
      <input type="checkbox" id="birth_place_unknown" name="birth_place_unknown">
      わからない（東京を使用）
    </label>
    <span class="error-message"></span>
  </div>

  <button type="submit">占う</button>
</form>
```

---

## 9. プロジェクト構造

```
fortune-telling/
├── app/
│   ├── __init__.py
│   ├── models.py              # SQLAlchemyモデル
│   ├── database.py            # DB接続・初期化
│   ├── routes.py              # Flaskルート定義
│   ├── fortune/
│   │   ├── __init__.py
│   │   ├── astrology.py       # 西洋占星術ロジック
│   │   ├── tarot.py           # タロットロジック
│   │   ├── numerology.py      # 数秘術ロジック
│   │   ├── shichu.py          # 四柱推命ロジック
│   │   └── iching.py          # 易経ロジック
│   ├── analyzer.py            # 総合分析エンジン
│   ├── templates/
│   │   ├── base.html
│   │   ├── index.html         # 入力フォーム
│   │   └── result.html        # 結果表示
│   └── static/
│       ├── css/
│       │   └── style.css
│       └── js/
│           └── main.js
├── data/
│   ├── fortune.db             # SQLiteデータベース
│   └── seed_data/             # 初期データ投入用
│       ├── tarot_data.json
│       ├── shichu_data.json
│       └── iching_data.json
├── tests/
│   ├── test_astrology.py
│   ├── test_tarot.py
│   └── ...
├── .kiro/
│   └── specs/
│       └── multi-method-fortune-telling/
│           ├── requirements.md  # 要件定義書
│           └── design.md        # 本設計書
├── requirements.txt
├── config.py
└── run.py                     # アプリケーション起動
```

---

## 10. 開発の流れ

### Phase 1実装ステップ

1. **環境セットアップ**
   - Python仮想環境作成
   - 必要なライブラリインストール（Flask, SQLAlchemy, pyswisseph等）

2. **データベース構築**
   - SQLiteデータベース作成
   - 全テーブル定義実行
   - 初期データ投入（マスターデータ）
     - タロットカード78枚
     - 易経64卦
     - 数秘術12個
     - 四柱推命マスター（十干・十二支・通変星等）
     - 星座・天体・ハウス・アスペクト

3. **入力バリデーション実装**
   - 生年月日のバリデーション
   - 出生時刻のバリデーション
   - 出生地のバリデーション
   - デフォルト値処理ロジック

4. **各占い手法の実装**
   - 西洋占星術モジュール（出生時刻・場所対応）
   - タロットモジュール（完全ランダム）
   - 数秘術モジュール
   - 四柱推命モジュール（時柱対応）
   - 易経モジュール

5. **総合分析エンジンの実装**
   - スコアリングロジック
   - 統合アルゴリズム
   - 共通点・相違点の分析ロジック
   - 計算根拠の生成

6. **Webアプリケーション実装**
   - Flaskルート
   - 入力フォーム（生年月日・出生時刻・出生地）
   - 結果表示画面（個別結果・総合判断・共通点/相違点）
   - 履歴保存機能
   - 履歴一覧・詳細表示

7. **視覚化の実装**
   - CSS スタイリング
   - プログレスバー、カラーコーディング
   - レスポンシブデザイン

8. **テスト**
   - 単体テスト（各占い手法、バリデーション）
   - 統合テスト（全体の流れ）
   - 実際のデータでの動作確認
   - デフォルト値使用時の動作確認

---

## 11. 技術的課題と解決策

### 課題1: 西洋占星術の天体計算の複雑さ
**解決策**: pyswissephライブラリの使用により、正確な天体位置計算を実現

### 課題2: タロットの解釈の多様性
**解決策**: データベースに複数の解釈パターンを格納し、カテゴリー別に適切な解釈を選択

### 課題3: 四柱推命の暦計算の正確性
**解決策**: 旧暦・節入り日の正確な計算ロジックを実装。必要に応じて外部ライブラリ（lunisolar等）を検討

### 課題4: データベースの初期データ量
**解決策**: JSON形式で初期データを準備し、スクリプトで一括投入。段階的にデータを充実させる

### 課題5: 総合判断の整合性
**解決策**: まずはシンプルな平均値ベースで実装。Phase 2でLLMを導入し、より洗練された解釈を生成

---

## 12. 非機能要件（参考）

### パフォーマンス
- 占い結果の生成時間: 5秒以内（目標）
- データベースクエリの最適化

### 拡張性
- 新しい占い手法の追加が容易
- データ駆動型設計により、コード変更を最小限に

### 保守性
- 明確なモジュール分割
- 命名規則の統一
- ドキュメント整備

### ローカル環境での動作
- インターネット接続不要（LLM未使用時）
- シンプルなセットアップ手順

---

## 13. 今後の拡張可能性

### Phase 2以降で検討する機能
- LLM統合（Claude/OpenAI APIによる自然な解釈文生成）
- グラフ・チャートの視覚化強化（Chart.js レーダーチャート）
- 計算根拠の詳細説明（歴史的背景、理論）
- 九星気学、宿曜占星術などの追加手法
- PDF出力機能
- 相性占い（2人の生年月日を比較）
- 時系列分析（過去の占い結果との比較）

### Phase 3以降
- 管理画面の実装
- 手法のON/OFF切り替え
- 手法ごとの重み付け設定
- カスタム解釈データの編集機能

---

## 付録: 主要ライブラリ

### 必須ライブラリ
```
Flask==3.0.0
SQLAlchemy==2.0.23
pyswisseph==2.10.3.2
```

### 推奨ライブラリ
```
python-dateutil==2.8.2
pytz==2023.3
```

### 将来的な追加（Phase 2以降）
```
anthropic==0.8.0          # Claude API
openai==1.6.0             # OpenAI API（選択肢）
```

---

## 改訂履歴

| 版 | 日付 | 変更内容 | 作成者 |
|---|------|---------|--------|
| 1.0 | 2025-11-06 | 初版作成 | Claude |
| 1.1 | 2025-11-06 | Phase 1スコープの大幅更新<br>- 出生時刻・出生地をPhase 1に追加（デフォルト値対応）<br>- 履歴保存機能をPhase 1に追加（最小限実装）<br>- 計算根拠の表示をPhase 1に追加<br>- 共通点・相違点の分析ロジックを詳細化<br>- タロットの完全ランダム仕様を明記<br>- 入力バリデーション仕様を追加<br>- デフォルト値処理の詳細を追加 | Claude |

